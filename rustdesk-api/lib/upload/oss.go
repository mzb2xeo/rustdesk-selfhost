...
/*  VerifySignature
*   VerifySignature requires three important pieces of data to verify the signature: 1> Get the public key PublicKey; 2> Generate a new MD5 authentication string; 3> Decode the authentication string carried by the Request;
*   1> Get the public key PublicKey: Get the URL from the "x-oss-pub-key-url" field of the RequestHeader, read the public key content contained in the URL link, decode and parse it, and use it as the input parameter for rsa.VerifyPKCS1v15.
*   2> Generate a new MD5 authentication string: Url decode the path part in the request url, add the query part of the url, and then add the body. After combination, perform MD5 encoding to get the MD5 authentication byte string.
*   3> Decode the authentication string carried by the Request: Get the "authorization" field of RequestHeader, decode it using Base64, and use it as the authentication comparison string for signature verification.
