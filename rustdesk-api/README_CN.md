...
| RUSTDESK_API_APP_CAPTCHA_THRESHOLD                     | 验证码触发次数; -1 不启用， 0 一真启用， >0 登录错误次数后启用 ;默认 `3`                                  | `3`                          |
...
2. **使用 `docker-compose` 运行 (推荐)**：
   项目根目录下已提供完整的 `docker-compose.yaml` 配置文件。它会自动：
   * 从本地源码编译 (build) API 服务 (包含 Web Admin 前端与 API 后端)。
   * 启动 RustDesk ID 服务器 (hbbs) 和 Relay 服务器 (hbbr)。
   * 启动 Nginx 做反向代理，处理 Web Client 的 HTTPS 以及 Websocket 链接。
   ...
   * 复制 `.env.example` 文件为 `.env`：
     ...
     cp .env.example .env
     ```
   * 修改 `.env` 文件，配置您的域名 (`DOMAIN`)、时区 (`TZ`) 和 JWT 密钥 (`JWT_SECRET`)。
   * 将您域名的 SSL 证书 (`fullchain.pem` 和 `privkey.pem`) 放置在 `./certs/` 目录下。
   * 运行以下命令编译并启动所有服务：
     ```bash
